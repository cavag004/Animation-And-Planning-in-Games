...
while (fringe.size() > 0){
  int fringeTop = 0;
  int fringeBottom = (fringe.size() - 1 >= 0) ? fringe.size() - 1 : 0;
  ...
  for (int i = 0; i < neighbors[currentNode].size(); i++){
    ...
    if (!visited[neighborNode]){
      ...
      boolean removeNeighborNode = true;
      for (int j = 0; j < neighbors[neighborNode].size(); j++) {
        int neighborChildNode = neighbors[neighborNode].get(j);
        if (visited[neighborChildNode] == false)
          removeNeighborNode = false;
      }
      if (removeNeighborNode == true) {
        fringe.remove(fringeBottom);
        println("Removed node", neighborNode, "from the fringe.");
        println(" Current Fringe: ", fringe); 
      }
    }
  }

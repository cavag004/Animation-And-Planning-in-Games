...
int closestNode(Vec2 point){
  ...
  //Vec2 circlePos[] = new Vec2[numObstacles];
  int closestNode = 0;
  float distanceToNode = -1.0;
  for (int i = 0; i < numNodes; i++) {
    if (point.distanceTo(nodePos[i]) < distanceToNode || distanceToNode == -1.0) {
      distanceToNode = point.distanceTo(nodePos[i]);
      closestNode = i;
    }
  }
  return closestNode;
...
boolean pointInBox(Vec2 boxTopLeft, float boxW, float boxH, Vec2 pointPos){
  ...
  float xmin = boxTopLeft.x;
  float xmax = boxTopLeft.x + boxW;
  float ymin = boxTopLeft.y;
  float ymax = boxTopLeft.y + boxH;
  
  if (pointPos.x >= xmin && pointPos.x <= xmax &&
      pointPos.y >= ymin && pointPos.y <= ymax) {
     return true;
  }
  return false;
}

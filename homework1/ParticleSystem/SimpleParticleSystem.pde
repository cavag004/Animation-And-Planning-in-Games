...
//     Step 6 does not have that issue, and that the red ball moves diagonally at the same speed it move horizontally or vertically.
...
void update(float dt){
  ...
  for (int i = 0; i < toGen; i++){
    ...
    colour[numParticles] = new Vec2(255, 0);
    numParticles += 1;
  }
  ...
  for (int i = 0; i <  numParticles; i++){
    ...
    if (colour[i].x == 0) {
      ...
      vel[i] = new Vec2(random(30, 90),random(-190,200)); 
      colour[i] = new Vec2(255, 0);
      break;
    }
    if (curTime > 0.25) {
      curTime = 0;
    ...
    if (pos[i].distanceTo(spherePos) < (sphereRadius+r)){
      ...
      velNormal.subtract(projAB(obstacleVel,normal));
      vel[i].subtract(velNormal.times(1 + COR));
      //vel[i].subtract(projAB(obstacleVel,normal));
    }
  }
...
void draw(){
  ...
  fill(0,0,255);
  for (int i = 0; i < numParticles; i++){
    fill(colour[i].x, colour[i].y, 255);
